...
def query_design_allocations_by_characteristic():
    ...
    if not results:
        print(
            "❌ No data found! epic_hours or project_characteristics table may be empty."
        )
        return

    print(
        f"{'Level':<8} {'Projects':<10} {'Avg %':<10} {'Min %':<10} {'Max %':<10} {'Avg Hours':<12} {'Avg Total':<12}"
    )
    ...
    for row in results:
        ...
        print(
            f"{level or 'NULL':<8} {count:<10} {avg_pct:<10.2f} {min_pct:<10.2f} {max_pct:<10.2f} {avg_hours:<12.0f} {avg_total:<12.0f}"
        )
    ...
    for row in results:
        ...
        if level == 3:
            if avg_pct < 10:
                print(
                    f"   ⚠️  custom_designs=3 has ONLY {avg_pct:.1f}% Design (expected 12-20%)"
                )
                print(
                    f"   → ROOT CAUSE: Historical projects systematically underfunded Design"
                )
            elif avg_pct >= 12 and avg_pct <= 20:
                print(
                    f"   ✅ custom_designs=3 has {avg_pct:.1f}% Design (within expected range)"
                )
                print(
                    f"   → AI should be allocating correctly. Check similar project selection."
                )
            else:
                print(
                    f"   🔍 custom_designs=3 has {avg_pct:.1f}% Design (outside expected 12-20%)"
                )
...
def simulate_similar_project_selection():
    ...
    print(
        f"{'Project':<15} {'Similarity':<12} {'Distance':<10} {'Design %':<10} {'Design h':<10} {'Characteristics':<40}"
    )
    ...
    for proj in top_5:
        ...
        print(
            f"{proj['project_key']:<15} {proj['similarity']:<12.4f} {proj['distance']:<10.2f} {proj['design_pct']:<10.2f} {proj['design_hours']:<10.0f} {chars_str:<40}"
        )
    ...
    if avg_design_pct < 10:
        print(
            f"   ⚠️  Similar projects have LOW Design allocation ({avg_design_pct:.1f}%)"
        )
        print(
            f"   → ROOT CAUSE: AI is learning from projects with historically low Design hours"
        )
    elif avg_design_pct >= 12 and avg_design_pct <= 20:
        print(
            f"   ✅ Similar projects have reasonable Design allocation ({avg_design_pct:.1f}%)"
        )
        print(
            f"   → AI should be allocating ~{avg_design_pct:.0f}% if following historical patterns"
        )
        print(f"   → If forecasts show 4%, AI is IGNORING historical data!")
    else:
        print(
            f"   🔍 Similar projects have {avg_design_pct:.1f}% Design (investigate further)"
        )

    # Check if any of top 5 have custom_designs != 3
    different_designs = [
        p for p in top_5 if p["characteristics"]["custom_designs"] != 3
    ]
    if different_designs:
        ...
        for p in different_designs:
            print(
                f"      - {p['project_key']}: custom_designs={p['characteristics']['custom_designs']} (Design: {p['design_pct']:.1f}%)"
            )
        print(
            f"   → Similar project selection may not be prioritizing custom_designs characteristic"
        )
...
def check_data_quality():
    ...
    if not results:
        print(
            "✅ No data quality issues found! All projects have non-zero Design and total hours."
        )
        return

    print(f"Found {len(results)} projects with data quality issues:\n")
    print(
        f"{'Project':<15} {'custom_designs':<15} {'Design Hours':<15} {'Total Hours':<15} {'Status':<30}"
    )
    ...
    for row in results:
        ...
        print(
            f"{proj_key:<15} {custom_designs or 'NULL':<15} {design_h or 0:<15.0f} {total_h or 0:<15.0f} {status:<30}"
        )
    ...
    if zero_design_count > 0:
        ...
        print(
            f"   → ROOT CAUSE: Data import issue or Design work categorized differently"
        )
...
def main():
    ...
    try:
        ...
        print(
            "     → Fix: Re-categorize historical data or adjust baseline expectations"
        )
        print()
