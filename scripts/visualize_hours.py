...
def create_monthly_trend(df, output_dir):
    ...
    plt.title(f'{df["Project"].iloc[0]} - Monthly Hours Trend', fontsize=16, fontweight='bold')
...
def create_epic_breakdown(df, output_dir, top_n=10):
    ...
    epic_totals = df.groupby(['Epic', 'Epic_Summary'])['Hours'].sum().sort_values(ascending=False).head(top_n)
    ...
    labels = [f"{epic[0]}\n{epic[1][:50]}..." if len(epic[1]) > 50 else f"{epic[0]}\n{epic[1]}"
              for epic in epic_totals.index]
    plt.yticks(range(len(epic_totals)), labels, fontsize=9)

    plt.xlabel('Total Hours', fontsize=12)
    plt.title(f'{df["Project"].iloc[0]} - Top {top_n} Epics by Hours', fontsize=16, fontweight='bold')
...
def create_heatmap(df, output_dir):
    ...
    pivot = df.pivot_table(values='Hours', index='Epic', columns='Month', aggfunc='sum', fill_value=0)
    ...
    sns.heatmap(pivot_filtered, annot=True, fmt='.0f', cmap='YlOrRd',
                cbar_kws={'label': 'Hours'}, linewidths=0.5)
    plt.title(f'{df["Project"].iloc[0]} - Hours Heatmap (Top 15 Epics)', fontsize=16, fontweight='bold')
    plt.xlabel('Month', fontsize=12)
...
def create_stacked_area(df, output_dir, top_n=10):
    ...
    epic_totals = df.groupby('Epic')['Hours'].sum().sort_values(ascending=False).head(top_n)
    top_epics = epic_totals.index.tolist()

    # Filter to top epics and pivot
    df_filtered = df[df['Epic'].isin(top_epics)]
    pivot = df_filtered.pivot_table(values='Hours', index='Month', columns='Epic', aggfunc='sum', fill_value=0)
    pivot = pivot.sort_index()

    plt.figure(figsize=(14, 8))
    pivot.plot(kind='area', stacked=True, alpha=0.7, ax=plt.gca())

    plt.title(f'{df["Project"].iloc[0]} - Hours Distribution Over Time (Top {top_n} Epics)',
...
def create_summary_stats(df, output_dir):
    ...
    summary.append("-" * 80)
    top_epics = df.groupby(['Epic', 'Epic_Summary'])['Hours'].sum().sort_values(ascending=False).head(10)
    ...
    for month in sorted(df['Month'].unique()):
        ...
        top_3 = month_df.groupby(['Epic', 'Epic_Summary'])['Hours'].sum().sort_values(ascending=False).head(3)
