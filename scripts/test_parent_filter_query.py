...
def test_parent_filter_query():
    ...
    try:
        ...
        logger.info(f"✅ Found {len(matches_1)} Notion documents with parent_id field\n")
        ...
        if matches_1:
            ...
            for i, match in enumerate(matches_1[:3], 1):
                ...
                logger.info(f"      parent_id: {metadata.get('parent_id', 'none')[:12] if metadata.get('parent_id') else 'None'}...")
                logger.info(f"      parent_type: {metadata.get('parent_type', 'none')}")
                logger.info("")
        else:
            logger.warning("⚠️  No documents with parent_id found - backfill may still be in progress\n")
        ...
        if matches_1:
            ...
            if test_parent_id:
                ...
                if matches_3:
                    parent_matches = [m for m in matches_3 if m.get('metadata', {}).get('page_id') == test_parent_id]
                    child_matches = [m for m in matches_3 if m.get('metadata', {}).get('parent_id') == test_parent_id]

                    logger.info(f"   • Parent page matches: {len(parent_matches)}")
                    logger.info(f"   • Child page matches: {len(child_matches)}\n")

                    logger.info("✅ SUCCESS: Parent-based filtering query syntax works!\n")
                else:
        ...
        logger.info(f"✅ Parent filter query: {'Working' if matches_1 else 'Waiting for backfill'}")

        if len(matches_1) > 0:
            percentage = (len(matches_1) / len(matches_2) * 100) if len(matches_2) > 0 else 0
            logger.info(f"✅ Backfill progress: {percentage:.1f}% of documents have parent_id")
