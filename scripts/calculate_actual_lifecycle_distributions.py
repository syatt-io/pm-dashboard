...
for team in ["BE Devs", "FE Devs", "Design", "UX", "PMs", "Data"]:
    ...
    print(
        f"  Ramp Up:   {avg_ramp_up:5.1f}% (range: {min(data['ramp_up']):.1f}% - {max(data['ramp_up']):.1f}%)"
    )
    print(
        f"  Busy Peak: {avg_busy:5.1f}% (range: {min(data['busy']):.1f}% - {max(data['busy']):.1f}%)"
    )
    print(
        f"  Ramp Down: {avg_ramp_down:5.1f}% (range: {min(data['ramp_down']):.1f}% - {max(data['ramp_down']):.1f}%)"
    )
    ...
    if "examples" in data:
        ...
        for ex in data["examples"][:3]:
            print(
                f"    {ex['project']}: {ex['months']} months, {ex['total_hours']:.0f}h total"
            )
            print(
                f"      → {ex['ramp_up']:.1f}% / {ex['busy']:.1f}% / {ex['ramp_down']:.1f}%"
            )
