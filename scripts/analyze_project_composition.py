...
class ProjectCompositionAnalyzer:
    ...
    CATEGORIES = {
        'PM/Oversight': [
            'project oversight', 'project management', 'support', 'oversight'
        ],
        'Design/UX': [
            'design', 'ux', 'ui', 'prototype', 'ideation', 'style guide'
        ],
        'Content/Modules': [
            'content sections', 'components', 'modules', 'sections'
        ],
        'Navigation': [
            'header', 'footer', 'mega menu', 'menu', 'navigation'
        ],
        'Product Pages': [
            'pdp', 'product detail', 'plp', 'product list', 'quick view'
        ],
        'Search/Browse': [
        ...
        'Account/Auth': [
            'account', 'login', 'auth', 'user', 'profile'
        ],
        'Integration': [
            'integration', 'api', 'migration', 'import', 'export', 'sync'
        ],
        'Infrastructure': [
            'globals', 'analytics', 'seo', 'ci', 'deployment', 'testing', 'uat'
        ],
        'Custom Features': [
            'scholarship', 'configurator', 'academic', 'bopis', 'registry',
            'wishlist', 'donation', 'bulk', 'custom'
        ]
    }
    ...
    def analyze_project_composition(self, project_key: str) -> Dict:
        ...
        composition = []
        for category in sorted(category_hours.keys(), key=lambda x: category_hours[x], reverse=True):
    ...
    def calculate_category_averages(self, analyses: List[Dict]) -> Dict[str, float]:
        ...
        for category in category_totals:
            averages[category] = round(category_totals[category] / category_counts[category], 1)
    ...
    def print_analysis(self, analyses: List[Dict], project_filter: str = None):
        ...
        avg_pm = sum(a['pm_percentage'] for a in analyses) / total_projects if total_projects > 0 else 0
        ...
        for analysis in analyses:
            ...
            print(f"{'PM Overhead:':<40} {analysis['pm_hours']:.1f}h ({analysis['pm_percentage']:.1f}%)")
            ...
            for cat_info in analysis['composition']:
                print(f"{cat_info['category']:<30} {cat_info['hours']:<12.1f} {cat_info['percentage']:<12.1f}% {cat_info['epic_count']}")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Analyze project composition by epic category')
