...
def analyze_by_characteristics():
    ...
    for row in epic_data:
        ...
        characteristic_stats['be_integrations'][chars['be_integrations']][team].append(hours)
        characteristic_stats['custom_theme'][chars['custom_theme']][team].append(hours)
        characteristic_stats['custom_designs'][chars['custom_designs']][team].append(hours)
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        for char_name, char_data in characteristic_stats.items():
            ...
            for value in [True, False]:
                ...
                print(f"\n{value_label}:")
                print(f"  {'Team':15} | {'Avg Hours':10} | {'Range':20} | {'Samples':8}")
                print(f"  {'-'*60}")
                ...
                for team in sorted(char_data[value].keys()):
                    ...
                    print(f"  {team:15} | {avg_hours:10.2f}h | {min_hours:6.2f}-{max_hours:6.2f}h | {sample_size:3} epics")
...
def build_multiplier_matrix():
    ...
    non_integration_data = session.query(
        EpicHours.project_key,
        EpicHours.epic_key,
        EpicHours.team,
        func.sum(EpicHours.hours).label('total_hours')
    ).filter(
        EpicHours.project_key.in_(['BIGO', 'BMBY', 'IRIS', 'BEVS'])
    ).group_by(
        EpicHours.project_key,
        EpicHours.epic_key,
        EpicHours.team
    ...
    integration_data = session.query(
        EpicHours.project_key,
        EpicHours.epic_key,
        EpicHours.team,
        func.sum(EpicHours.hours).label('total_hours')
    ).filter(
        EpicHours.project_key.in_(['SRLK', 'COOP', 'CAR'])
    ).group_by(
        EpicHours.project_key,
        EpicHours.epic_key,
        EpicHours.team
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        for team in sorted(baseline_avg.keys()):
            ...
            if base > 0 and with_integration > 0:
                ...
                print(f"{team:15} | {base:10.2f}h | {with_integration:12.2f}h | {multiplier:10.2f}x | +{increase:8.2f}h")
...
def build_forecasting_template():
    ...
    no_int_records = session.query(EpicHours).filter(
        EpicHours.project_key.in_(['BIGO', 'BMBY', 'IRIS', 'BEVS'])
    ).all()
    ...
    int_records = session.query(EpicHours).filter(
        EpicHours.project_key.in_(['SRLK', 'COOP', 'CAR'])
    ).all()
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        for team in sorted(team_baselines.keys()):
            ...
            lifecycle = lifecycle_pcts.get(team, {'ramp_up': 50, 'busy': 35, 'ramp_down': 15})
            ...
            print(f"{team:15} | {no_int:10.2f}h | {with_int:10.2f}h | {lifecycle['ramp_up']:.1f}% / {lifecycle['busy']:.1f}% / {lifecycle['ramp_down']:.1f}%")
