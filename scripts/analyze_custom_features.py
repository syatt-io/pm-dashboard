...
class CustomFeatureAnalyzer:
            ...
            if project_count == 1 or not is_in_baseline:
                for project_info in data['projects']:
                    custom_epics_by_project[project_info['project_key']].append({
                        ...
                        'avg_hours_per_occurrence': project_info['hours'] / project_info['occurrences'],
                        'is_high_risk': project_info['hours'] > self.HIGH_RISK_THRESHOLD,
                        'reason': 'unique_to_project' if project_count == 1 else 'not_in_baseline'
                    })
    ...
    def calculate_complexity_score(self, project_key: str) -> Dict:
        ...
        return {
            ...
            'custom_epics': sorted(custom_epics, key=lambda x: x['hours'], reverse=True)
        }
    ...
    def print_analysis(self, analysis: List[Dict], project_filter: str = None):
        ...
        avg_custom_pct = sum(a['custom_percentage'] for a in analysis) / total_projects if total_projects > 0 else 0
        ...
        for project in analysis:
            print(f"\n{'=' * 100}")
            print(f"PROJECT: {project['project_key']} (Complexity: {project['complexity_level'].upper()})")
            print(f"{'=' * 100}")
            print(f"{'Total Hours:':<40} {project['total_hours']:.1f}h")
            print(f"{'Custom Hours:':<40} {project['custom_hours']:.1f}h ({project['custom_percentage']:.1f}%)")
            print(f"{'Custom Epics:':<40} {project['custom_epic_count']}")
            print(f"{'High-Risk Custom Epics (>50h):':<40} {project['high_risk_epic_count']} ({project['high_risk_hours']:.1f}h)")
            ...
            if project['custom_epics']:
                ...
                for epic in project['custom_epics'][:10]:  # Top 10
                    ...
                    reason = "Unique" if epic['reason'] == 'unique_to_project' else "Not in baseline"
                    print(f"{epic['epic_summary'][:48]:<50} {epic['hours']:<10.1f} {risk:<10} {reason}")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Analyze custom features and project complexity')
