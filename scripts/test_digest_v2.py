...
def generate_digest(
    project_key: str, days: int, prompt_config_path: str, include_context: bool = True
) -> dict:
    """
    ...
    try:
        ...
        result = session.execute(
            text("SELECT name FROM projects WHERE key = :key"), {"key": project_key}
        ).first()
...
def create_comparison_markdown(
    old_result: dict, new_result: dict, project_key: str, days: int
) -> str:
...
def main():
    ...
    parser.add_argument(
        "--project", type=str, required=True, help="Project key (e.g., SUBS, SATG)"
    )
    parser.add_argument(
