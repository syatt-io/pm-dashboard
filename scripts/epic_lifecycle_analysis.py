...
def analyze_integration_impact():
    ...
    integration_stats = defaultdict(lambda: defaultdict(lambda: {'hours': 0, 'epics': set()}))
    ...
    for row in all_data:
        ...
        integration_stats[category][row.team]['epics'].add(f"{row.project_key}-{row.epic_key}")

    # Calculate percentages and write to CSV
    csv_path = output_dir / '1_integration_impact.csv'
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Project_Category', 'Team', 'Total_Hours', 'Num_Epics', 'Avg_Hours_Per_Epic', 'Percent_Of_Total'])
        ...
        for category in sorted(integration_stats.keys()):
            ...
            total_hours = sum(data['hours'] for data in integration_stats[category].values())
            ...
            for team in sorted(integration_stats[category].keys()):
                ...
                print(f"  {team:15} | {hours:10.2f}h | {num_epics:3} epics | {percent:5.1f}%")

    print(f"\n✅ Saved to: {csv_path}")

    # Key insights
    print("\n💡 KEY INSIGHTS:")
    integration_be_pct = integration_stats['Integration Required']['BE Devs']['hours'] / sum(d['hours'] for d in integration_stats['Integration Required'].values()) * 100
    no_integration_be_pct = integration_stats['No Integration']['BE Devs']['hours'] / sum(d['hours'] for d in integration_stats['No Integration'].values()) * 100 if 'BE Devs' in integration_stats['No Integration'] else 0
    ...
    print(f"   • Backend multiplier: {integration_be_pct / no_integration_be_pct if no_integration_be_pct > 0 else 'N/A'}x more backend work with integrations")
...
def analyze_epic_lifecycle_patterns():
    ...
    with open(csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Epic', 'Team', 'Total_Months', 'Total_Hours', 'Month_Num', 'Phase', 'Percent_Of_Total'])
        ...
        for team in sorted(team_phase_patterns.keys()):
            ...
            for phase in ['Ramp Up', 'Busy (Peak)', 'Ramp Down']:
                ...
                if pcts:
                    ...
                    print(f"  {phase:15} | {avg_pct:6.1f}% | {min_pct:.1f}%-{max_pct:.1f}% (n={sample_size})")
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        for category in ['Integration Required', 'No Integration']:
            ...
            for team in sorted(baseline_data[category].keys()):
                ...
                print(f"  {team:15} | {avg_hours:8.2f}h | {sample_size:3} epics | {lifecycle}")
