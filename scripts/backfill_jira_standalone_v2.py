    ...
    for cache_file in CACHE_DIR.glob("*.json"):
        try:
            with open(cache_file, 'r') as f:
                ...
                logger.info(f"📂 Loaded {len(data['issues'])} cached issues for {project_key}")
        except Exception as e:
...
async def backfill_jira_issues(days_back: int = 730, resume: bool = True, project_filter: List[str] = None, active_only: bool = False) -> Dict[str, Any]:
    ...
    if already_fetched and resume:
        logger.info(f"📂 Found {len(already_fetched)} already fetched projects: {', '.join(sorted(already_fetched))}")
    ...
    for i, project in enumerate(projects, 1):
        ...
        if resume and project_key in already_fetched:
            logger.info(f"⏭️  [{i}/{len(projects)}] Skipping {project_key} (already cached)")
            projects_skipped += 1
            continue
        ...
        try:
            ...
            if project_issues:
                ...
                logger.info(f"   ✅ [{i}/{len(projects)}] {project_key}: {len(project_issues)} issues (saved to disk)")
            else:
    ...
    logger.info(f"✅ Loaded {total_issues} total issues from {len(all_cached_data)} projects")
    logger.info(f"📊 Projects: {', '.join(f'{k} ({len(v)})' for k, v in sorted(all_cached_data.items()))}")

    # Ingest into Pinecone
    logger.info(f"\n{'='*80}")
    logger.info(f"📥 Ingesting {total_issues} issues from {len(all_cached_data)} projects into Pinecone...")
    logger.info(f"{'='*80}\n")

    total_ingested = 0
    for i, (project_key, project_issues) in enumerate(sorted(all_cached_data.items()), 1):
        try:
            # Add delay between projects (except first one)
            if i > 1:
                logger.info(f"   ⏱️  Waiting {DELAY_BETWEEN_PROJECTS}s before next project...")
            ...
            logger.info(f"[{i}/{len(all_cached_data)}] Ingesting {len(project_issues)} issues from {project_key}...")
            count = ingest_service.ingest_jira_issues(
                issues=project_issues,
                project_key=project_key
            )
            total_ingested += count
            logger.info(f"✅ [{i}/{len(all_cached_data)}] Ingested {count} issues from {project_key} (Total: {total_ingested}/{total_issues})")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Backfill Jira issues to Pinecone (with disk caching)')
