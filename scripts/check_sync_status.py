...
try:
    with engine.connect() as conn:
        ...
        if not table_exists:
            ...
            print(
                "This suggests vector ingestion has never been set up or the migration hasn't run."
            )
            exit(1)
        ...
        if not rows:
            ...
            print(
                "This means vector ingestion tasks have never successfully completed."
            )
        else:
            ...
            for row in rows:
                ...
                if last_sync:
                    ...
                    print(
                        f"{emoji} {source:<14} {last_sync.strftime('%Y-%m-%d %H:%M:%S UTC'):<30} {age_str}"
                    )
                else:
            ...
            print("\n📊 Summary:")
            stale_sources = [
                r[0] for r in rows if r[1] and (datetime.now() - r[1]).days >= 1
            ]
            if stale_sources:
