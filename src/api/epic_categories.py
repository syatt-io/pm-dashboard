...
epic_categories_bp = Blueprint(
    "epic_categories", __name__, url_prefix="/api/epic-categories"
)
...
def create_category(user):
    ...
    try:
        ...
        if not data or "name" not in data:
            return (
                jsonify({"success": False, "error": "Missing required field: name"}),
                400,
            )
        ...
        if "display_order" in data:
            ...
            max_order = (
                session.query(EpicCategory.display_order)
                .order_by(EpicCategory.display_order.desc())
                .first()
            )
            display_order = (
        ...
        return jsonify({"success": True, "category": category_dict}), 201
...
def update_category(user, category_id):
    ...
    try:
        ...
        if "name" in data:
            ...
            existing = (
                session.query(EpicCategory)
                .filter(EpicCategory.name == new_name, EpicCategory.id != category_id)
                .first()
            )
        ...
        logger.info(
            f"Epic category updated: {category_dict['name']} (id: {category_id})"
        )

        return jsonify({"success": True, "category": category_dict}), 200
...
def delete_category(user, category_id):
    ...
    try:
        ...
        mappings_count = (
            session.query(EpicCategoryMapping).filter_by(category=category.name).count()
        )
...
def reorder_categories(user):
    ...
    try:
        ...
        if not data or "categories" not in data:
            return (
                jsonify(
                    {"success": False, "error": "Missing required field: categories"}
                ),
                400,
            )
        ...
        if not isinstance(categories_data, list):
            return (
                jsonify({"success": False, "error": "Categories must be an array"}),
                400,
            )
...
def get_categories():
    ...
    try:
        ...
        return jsonify({"success": True, "categories": category_names}), 200
...
def delete_mapping(epic_key):
    ...
    try:
        ...
        return (
            jsonify({"success": True, "epic_key": epic_key, "action": "deleted"}),
            200,
        )
