...
def get_baselines(user):
    ...
    try:
        ...
        baselines = query.order_by(
            EpicBaseline.project_count.desc(), EpicBaseline.median_hours.desc()
        ).all()
                ...
                epic_estimates.append(
                    {
                        ...
                        "confidence": (
                            "high" if baseline.variance_level == "low" else "medium"
                        ),
...
def get_projects(user):
    ...
    try:
        ...
        results = (
            ...
            .order_by(func.sum(EpicHours.hours).desc())
            .all()
        )
            ...
            composition.append(
                {
                    ...
                    "hours": round(hours, 1),
                    "percentage": round(
                        (hours / total_hours * 100) if total_hours > 0 else 0, 1
                    ),
                }
            )
...
def get_high_variance_epics(user):
    ...
    try:
        ...
        high_variance = (
            session.query(EpicBaseline)
            .filter(EpicBaseline.variance_level == "high")
            .order_by(EpicBaseline.coefficient_of_variation.desc())
            .all()
        )
...
def export_project_schedule(user):
    ...
    try:
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename=project_schedule_{start_date}.csv"
        )
    ...
    try:
        ...
        for script_info in scripts:
            ...
            try:
                ...
                results[script_name] = {"success": False, "output": "", "error": str(e)}
                all_success = False
        ...
        return jsonify(
            {
                ...
                "message": (
                    ...
                    if all_success
                    else "Some analysis scripts failed"
                ),
                "results": results,
