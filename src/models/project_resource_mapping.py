...
class ProjectResourceMapping(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "slack_channel_ids": (
                self.slack_channel_ids.split(",") if self.slack_channel_ids else []
            ),
            "notion_page_ids": (
                self.notion_page_ids.split(",") if self.notion_page_ids else []
            ),
            "github_repos": self.github_repos.split(",") if self.github_repos else [],
            "jira_project_keys": (
                self.jira_project_keys.split(",") if self.jira_project_keys else []
            ),
            "internal_slack_channels": (
                self.internal_slack_channels.split(",")
                if self.internal_slack_channels
                else []
            ),
