...
class ProcessedMeetingDTO:
    ...
    def from_orm(cls, meeting):
        ...
        return cls(
            ...
            # Legacy structure fields (DEPRECATED - for backward compatibility)
            executive_summary=getattr(meeting, "executive_summary", None)
            or meeting.summary,
            outcomes=parse_json_field(getattr(meeting, "outcomes", None), [])
            or parse_json_field(meeting.key_decisions, []),
            blockers_and_constraints=parse_json_field(
                getattr(meeting, "blockers_and_constraints", None), []
            )
            or parse_json_field(meeting.blockers, []),
            timeline_and_milestones=parse_json_field(
                getattr(meeting, "timeline_and_milestones", None), []
            ),
            key_discussions=parse_json_field(
                getattr(meeting, "key_discussions", None), []
            ),
            # Legacy fields for backward compatibility
