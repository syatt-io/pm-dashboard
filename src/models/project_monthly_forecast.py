...
class ProjectMonthlyForecast(Base):
    ...
    project_key = Column(
        String(50),
        ForeignKey("projects.key", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "forecasted_hours": (
                float(self.forecasted_hours) if self.forecasted_hours else 0
            ),
            "actual_monthly_hours": (
                float(self.actual_monthly_hours) if self.actual_monthly_hours else 0
            ),
