...
class MeetingMetadata(Base):
    ...
    normalized_title = Column(String(255), nullable=False, index=True)  # Lowercase, normalized for matching
    meeting_type = Column(String(50), nullable=True, index=True)  # 'standup', 'weekly_sync', 'client', 'planning', 'other'
    ...
    def normalize_title(title: str) -> str:
        ...
        normalized = re.sub(r'\d{1,2}:\d{2}\s*(am|pm)?', '', normalized, flags=re.IGNORECASE)
    ...
    def detect_meeting_type(title: str) -> str:
        ...
        # Standup patterns
        if any(keyword in title_lower for keyword in ['standup', 'daily', 'scrum', 'check-in', 'checkin']):
            return 'standup'

        # Weekly sync patterns
        if any(keyword in title_lower for keyword in ['weekly', 'week', 'sync', 'status']):
            return 'weekly_sync'

        # Client meeting patterns
        if any(keyword in title_lower for keyword in ['client', 'customer', 'stakeholder', 'demo', 'review']):
            return 'client'

        # Planning patterns
        if any(keyword in title_lower for keyword in ['planning', 'sprint planning', 'roadmap', 'backlog']):
            return 'planning'
