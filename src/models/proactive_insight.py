...
class ProactiveInsight(Base):
    ...
    )  # 'created_ticket', 'ignored', 'resolved', etc.
    delivered_via_slack = Column(
        DateTime, nullable=True
    )  # Track when delivered via Slack
    delivered_via_email = Column(
        DateTime, nullable=True
    )  # Track when delivered via Email

    # Escalation tracking fields
    last_escalated_at = Column(
        DateTime, nullable=True, index=True
    )  # When last escalation occurred
    ...
    escalation_history = relationship(
        "EscalationHistory", back_populates="insight", cascade="all, delete-orphan"
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "delivered_via_slack": (
                self.delivered_via_slack.isoformat()
                if self.delivered_via_slack
                else None
            ),
            "delivered_via_email": (
                self.delivered_via_email.isoformat()
                if self.delivered_via_email
                else None
            ),
            "last_escalated_at": (
    ...
    def is_delivered(self):
        ...
        return (
            self.delivered_via_slack is not None or self.delivered_via_email is not None
        )
