...
class EpicBaseline(Base):
    ...
    # Metadata
    project_count = Column(Integer, nullable=False)  # Number of projects using this epic
    occurrence_count = Column(Integer, nullable=False)  # Total occurrences across all projects
    coefficient_of_variation = Column(Float, nullable=False)  # CV% = (std_dev / mean) * 100
