...
from src.models import ProactiveInsight, User, UserWatchedProject, ProcessedMeeting, MeetingMetadata
...
class InsightDetector:
    ...
    def _detect_anomaly(self, user: User, project_keys: List[str]) -> List[ProactiveInsight]:
        ...
        try:
            ...
            for project_key in project_keys:
                try:
                    ...
                    hours_anomaly = self._detect_hours_anomaly(project_key, tempo_client)
                    if hours_anomaly:
                        insight = ProactiveInsight(
                            id=str(uuid.uuid4()),
                            user_id=user.id,
                    ...
                    logger.error(f"Error detecting anomalies for project {project_key}: {e}")
                    continue
    ...
    def _detect_hours_anomaly(self, project_key: str, tempo_client: Optional[Any]) -> Optional[Dict]:
        ...
        try:
            ...
            if baseline_hours > 0:
                deviation_pct = ((current_week_hours - baseline_hours) / baseline_hours) * 100
            else:
                deviation_pct = 0
            ...
            if abs(deviation_pct) >= THRESHOLD:
                ...
                return {
                    ...
                    'metadata': {
                        ...
                        'anomaly_type': 'hours_spike' if deviation_pct > 0 else 'hours_drop',
    ...
    def _detect_velocity_anomaly(self, project_key: str) -> Optional[Dict]:
        ...
        try:
            ...
            baseline_tickets = sum(r.tickets_closed for r in result[1:]) / len(result[1:]) if len(result) > 1 else 0
            ...
            if baseline_tickets > 0:
                deviation_pct = ((current_week_tickets - baseline_tickets) / baseline_tickets) * 100
            else:
                deviation_pct = 0
    ...
    def _detect_meeting_anomaly(self, project_key: str) -> Optional[Dict]:
        ...
        try:
            ...
            recurring_meetings = self.db.query(MeetingMetadata).filter(
            ...
            for meeting in recurring_meetings:
                try:
                    ...
                    if days_overdue >= 2:
                        ...
                        recent_meetings = self.db.query(ProcessedMeeting).filter(
                            ProcessedMeeting.title.ilike(f"%{meeting.normalized_title}%")
                        ).order_by(desc(ProcessedMeeting.date)).limit(5).all()
                        ...
                        if recent_meetings:
                            ...
                            expected_meetings = (now - last_meeting_date).days // expected_interval_days
                        ...
                        if skipped_count >= 2:
                            severity = 'warning'
                            title = f"{project_key}: Recurring meeting pattern disrupted"
                            description = (
                            ...
                            return {
                                ...
                                'metadata': {
                                    ...
                                    'last_occurrence': meeting.last_occurrence.isoformat() if meeting.last_occurrence else None,
