...
class MeetingProjectLinker:
    ...
    def _save_meeting_project_connections(
        self,
        meeting_id: str,
        meeting_title: str,
        meeting_date: datetime,
        project_relevance: List[Dict[str, Any]],
    ) -> None:
        ...
        try:
            ...
            session.query(MeetingProjectConnection).filter_by(
            ...
            session.close()
            logger.info(
                f"Saved {len([p for p in project_relevance if p.get('relevance_score', 0) > 1.0])} connections for meeting {meeting_id}"
            )
    ...
    async def analyze_meeting_project_relevance(
        self, meeting_id: str
    ) -> Dict[str, Any]:
    ...
    async def _calculate_project_relevance(
        self, transcript, analysis, projects
    ) -> List[Dict[str, Any]]:
        """Calculate relevance score for each project based on meeting content."""
        project_scores = []

        for project in projects:
            score = await self._score_project_relevance(project, transcript, analysis)
    ...
    async def _score_project_relevance(self, project, transcript, analysis) -> float:
        ...
        for word in name_words:
            if len(word) > 3 and word in title_lower and word not in common_words:
                ...
                matching_factors.append(
                    f"Common word '{word}' in meeting title (low confidence)"
                )
        ...
        additional_score, additional_factors = self._check_title_patterns(
            title_lower, project_key, project_name
        )
    ...
    def _check_title_patterns(
        self, title_lower: str, project_key: str, project_name: str
        ...
        for meeting_type in meeting_types:
            if meeting_type in title_lower and (
                project_key in title_lower
                or any(
                    word in title_lower
                    for word in project_name.lower().split()
                    if len(word) > 3
                )
            ):
                score += 3.0
                matching_factors.append(
                    f"Project-related meeting type: '{meeting_type}'"
                )
                break
    ...
    def _get_top_suggestions(
        self, project_relevance: List[Dict[str, Any]], limit: int = 3
    ) -> List[Dict[str, Any]]:
        """Get top project suggestions based on relevance."""
        return project_relevance[:limit]

    async def get_meetings_for_projects(
        self, project_keys: List[str], days_back: int = 30
        ...
        try:
            ...
            try:
                cached_connections = (
                    session.query(MeetingProjectConnection)
                    .filter(
                ...
                logger.info(
                    f"Found {len(cached_connections)} cached connections for projects {project_keys}"
                )

                # Get all recent meetings from Fireflies to check for missing ones
                meetings = self.fireflies_client.get_recent_meetings(
                    days_back=days_back
                )
                ...
                missing_meetings = [
                    m for m in meetings if m.get("id") not in cached_meeting_ids
                ]
                ...
                for conn in cached_connections:
                    meeting_summary = {
                        ...
                        "date": int(
                            conn.meeting_date.timestamp() * 1000
                ...
                if missing_meetings:
                    logger.info(
                        f"Analyzing {len(missing_meetings)} missing meetings..."
                    )
                    for meeting in missing_meetings:
                        ...
                        try:
                            ...
                            transcript = self.fireflies_client.get_meeting_transcript(
                                meeting_id
                            )
                            ...
                            if project_relevance:
                                ...
                                for proj_data in project_relevance:
                                    if (
                                        proj_data["project_key"] in project_keys
                                        and proj_data["relevance_score"] > 5.0
                                    ):
                                        meeting_summary = {
                                            "meeting_id": meeting_id,
                                            "title": transcript.title,
                                            "date": meeting.get("date", 0),
                                            "relevance_score": proj_data[
                                                "relevance_score"
                                            ],
                                        ...
                                        results[proj_data["project_key"]].append(
                                            meeting_summary
                                        )

                        except Exception as meeting_error:
                            logger.warning(
                                f"Error analyzing meeting {meeting_id}: {meeting_error}"
                            )
                            continue

                # Sort meetings by relevance score within each project
                for project_key in results:
                    results[project_key].sort(
                ...
                logger.warning(
                    f"Error with database, returning empty results: {db_error}"
                )
                session.close()
                return {}
    ...
    async def suggest_project_actions(
        self, project_key: str, days_back: int = 30
    ) -> Dict[str, Any]:
        """Suggest actions for a project based on recent meeting discussions."""
        try:
            # Get meetings related to this project
            project_meetings = await self.get_meetings_for_projects(
                [project_key], days_back
            )
            ...
            for meeting in meetings[:5]:  # Analyze top 5 most relevant meetings
                ...
                if transcript:
                    ...
                    project_actions = [
                        ...
                        if project_key.lower()
                        in f"{action.title} {action.description}".lower()
                    ]
                            ...
                            [
                                {
                                    ...
                                    "meeting_date": transcript.date.strftime(
                                        "%Y-%m-%d"
                                    ),
                                }
                    ...
                    recent_discussions.append(
                        {
                            ...
                            "summary": (
                                analysis.executive_summary[:200] + "..."
                                if len(analysis.executive_summary) > 200
    ...
    async def create_project_meeting_dashboard_data(
        self, project_keys: List[str], days_back: int = 7
    ) -> Dict[str, Any]:
        """Create dashboard data showing meeting-project relationships."""
        try:
            # Get meetings for all projects
            project_meetings = await self.get_meetings_for_projects(
                project_keys, days_back=days_back
            )

            # Calculate summary statistics
            total_meetings = sum(
                len(meetings) for meetings in project_meetings.values()
            )
                            ...
                            cross_project_meetings.append(
                                {
                                    ...
                                    "total_relevance": sum(
                                        p["relevance_score"] for p in projects
                                    ),
                                }
                            )
                            break
