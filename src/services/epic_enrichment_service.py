...
class EpicEnrichmentService:
    ...
    def __init__(self, session: Session):
        ...
        if not all([self.jira_url, self.jira_username, self.jira_api_token]):
            logger.warning(
                "Jira credentials not configured - enrichment will be skipped"
            )

    def get_epics_needing_enrichment(
        ...
        query = self.session.query(
            EpicHours.project_key, EpicHours.epic_key, EpicHours.epic_summary
        ).distinct()
        ...
        logger.info(
            f"Found {total_epics} epics across {len(epics_by_project)} projects needing enrichment"
        )

        return dict(epics_by_project)

    def fetch_epic_summaries_from_jira(
        self, epics_by_project: Dict[str, Set[str]]
    ) -> Dict[str, str]:
                ...
                for epic_key in sorted(epic_keys):
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if (
                                "fields" in epic_data
                                and "summary" in epic_data["fields"]
                            ):
                                ...
                                logger.warning(
                                    f"  {epic_key}: No summary found, keeping original"
                                )
                                epic_summaries[epic_key] = epic_key
                        elif response.status_code == 404:
                            logger.warning(
                                f"  {epic_key}: Not found in Jira (404), keeping original"
                            )
                            epic_summaries[epic_key] = epic_key
                        else:
                            logger.error(
                                f"  {epic_key}: HTTP {response.status_code}, keeping original"
                            )
                            epic_summaries[epic_key] = epic_key
