...
class ProjectNotificationService:
    ...
    def _generate_notification_content(self, changes: List[Dict[str, Any]],
                                     selected_projects: List[str],
                                     cadence: str) -> Dict[str, str]:
        ...
        # Overview section
        body_parts.append(f"*{total_changes} total changes* across *{projects_with_changes} projects* {period_text}")
        body_parts.append("")

        # Change type breakdown
        if change_type_counts:
            body_parts.append("*📈 Change Breakdown:*")
            for change_type, count in sorted(change_type_counts.items(), key=lambda x: x[1], reverse=True):
        ...
        for project_key in sorted(project_changes.keys()):
            ...
            project_total = sum(len(changes) for changes in project_change_types.values())
            ...
            for change in all_project_changes[:5]:
                ...
                if change['change_type'] == 'created':
                    body_parts.append(f"  {emoji} {time_str} - New: {change['ticket_key']} - {change['ticket_title'][:50]}")
                elif change['change_type'] == 'status_changed':
                    body_parts.append(f"  {emoji} {time_str} - Status: {change['ticket_key']} → {change['new_value']}")
                elif change['change_type'] == 'assignee_changed':
                    body_parts.append(f"  {emoji} {time_str} - Assigned: {change['ticket_key']} → {change['new_value']}")
                else:
                    readable_type = self._get_readable_change_type(change['change_type'])
                    body_parts.append(f"  {emoji} {time_str} - {readable_type}: {change['ticket_key']}")

            if len(all_project_changes) > 5:
                body_parts.append(f"  ... and {len(all_project_changes) - 5} more changes")
            body_parts.append("")

        # Action items section
        if any(change['change_type'] == 'assignee_changed' for change in changes):
            body_parts.append("*🎯 Action Items:*")
            assigned_to_me = [c for c in changes if c['change_type'] == 'assignee_changed'
                            and c['new_value'] and 'you' in c['new_value'].lower()]
            if assigned_to_me:
                body_parts.append(f"  • {len(assigned_to_me)} tickets newly assigned")

            overdue_mentions = [c for c in changes if 'overdue' in str(c.get('change_details', {})).lower()]
            if overdue_mentions:
                body_parts.append(f"  • Check {len(overdue_mentions)} tickets mentioned as overdue")
            body_parts.append("")
    ...
    async def _send_slack_dm(self, slack_user_id: str, content: Dict[str, str]):
        ...
        try:
            ...
            logger.info(f"Would send DM to Slack user {slack_user_id}: {content['title']}")
    ...
    async def send_daily_notifications(self):
        ...
        try:
            ...
            daily_users = db_session.query(UserPreference).filter_by(notification_cadence='daily').all()
            ...
            logger.info(f"Sent daily notifications to {sent_count}/{len(daily_users)} users")
    ...
    async def send_weekly_notifications(self):
        ...
        try:
            ...
            weekly_users = db_session.query(UserPreference).filter_by(notification_cadence='weekly').all()
            ...
            logger.info(f"Sent weekly notifications to {sent_count}/{len(weekly_users)} users")
    ...
    async def send_monthly_notifications(self):
        ...
        try:
            ...
            monthly_users = db_session.query(UserPreference).filter_by(notification_cadence='monthly').all()
            ...
            logger.info(f"Sent monthly notifications to {sent_count}/{len(monthly_users)} users")
