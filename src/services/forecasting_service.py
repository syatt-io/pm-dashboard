...
class ForecastingService:
    ...
    def _distribute_hours_by_month(
        ...
        # Proration handles mid-month starts correctly:
        # - Reduces first month hours for partial month
        # - Redistributes those hours to remaining months
        # - Normalization ensures total budget is maintained
        distributions = self.temporal_pattern_service.distribute_hours_by_month(
            total_hours=total_hours,
            team=team_name,
            duration_months=num_months,
            start_date=start_date_obj,
            prorate_first_month=True,  # Re-enabled with normalization fix
            prorate_last_month=True,  # Re-enabled with normalization fix
        )
