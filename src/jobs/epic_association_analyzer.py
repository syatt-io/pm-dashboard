...
class EpicAssociationAnalyzer:
    ...
    async def get_unassigned_tickets(self, project_key: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Error fetching unassigned tickets for {project_key}: {e}",
                exc_info=True,
            )
            return []
    ...
    async def update_ticket_epic_link(self, ticket_key: str, epic_key: str) -> bool:
        ...
        try:
            ...
            updates = {"fields": {"customfield_10014": epic_key}}
    ...
    async def analyze_project(
        self, project_key: str, auto_update: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if not epics:
            logger.warning(
                f"No epics found in {project_key}, marking tickets as NO_MATCH"
            )
        ...
        matches = self.epic_matcher.batch_match_tickets(
            tickets=tickets, available_epics=epics, confidence_threshold=0.5
        )
        ...
        if auto_update and matches:
            ...
            for match in matches:
                success = await self.update_ticket_epic_link(
                    ticket_key=match["ticket_key"], epic_key=match["suggested_epic_key"]
                )
                if success:
                    updates_applied += 1
                else:
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Auto-update mode: {'ENABLED ✅' if auto_update else 'DISABLED (Summary only)'}"
        )
        logger.info("")
        ...
        logger.info(
            f"  - Medium confidence (0.5-0.79): {summary['medium_confidence_matches']}"
        )
    ...
    def _save_detailed_results(
        self, all_matches: List[Dict[str, Any]], project_results: List[Dict[str, Any]]
    ) -> None:
        ...
        try:
            ...
            detailed_file = Path(
                "/Users/msamimi/syatt/projects/agent-pm/epic_matches_detailed.csv"
            )
            ...
            with open(detailed_file, "w", newline="") as f:
                ...
                for match in sorted(
                    all_matches, key=lambda x: (-x["confidence"], x["ticket_key"])
                ):
                    level = (
                        "HIGH"
                        if match["confidence"] >= 0.8
                        else ("MEDIUM" if match["confidence"] >= 0.5 else "LOW")
                    )
                    writer.writerow(
                        [
                            match["ticket_key"],
            ...
            summary_file = Path(
                "/Users/msamimi/syatt/projects/agent-pm/epic_matches_by_project.csv"
            )
    ...
    def send_slack_notification(self, summary: Dict[str, Any]) -> None:
        ...
        try:
            ...
            message_blocks.append(
                {
                    "type": "context",
                    "elements": [{"type": "mrkdwn", "text": mode_explanation}],
                }
            )
            ...
            if summary.get("projects"):
                ...
                for project in summary["projects"]:
                    ...
                    if matches > 0:
                        updates_text = (
                            f" ({project['updates_applied']} applied)"
                            if auto_update
                            else ""
                        )
                        project_details.append(
                            f"• *{proj_key}:* {matches} matches{updates_text}"
                        )
