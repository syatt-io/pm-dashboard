...
def get_dashboard_stats(user):
    ...
    try:
        ...
        with session_scope() as db_session:
            ...
            total_meetings = (
                db_session.query(func.count(ProcessedMeeting.id)).scalar() or 0
            )
            ...
            if user.role.value != "admin":
                ...
                watched_project_keys = [
                    wp.project_key
                    for wp in db_session.query(UserWatchedProject)
                ...
                total_todos = (
                    db_session.query(func.count(TodoItem.id))
                ...
                completed_todos = (
                    db_session.query(func.count(TodoItem.id))
                    .filter(visibility_filter, TodoItem.status == "done")
                    .scalar()
                    or 0
                )
            else:
            ...
            try:
                ...
                total_projects = (
                    db_session.query(func.count(JiraProject.key))
                    .filter(JiraProject.is_active == True)
                    .scalar()
                    or 0
                )
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            try:
                ...
                return (
                    jsonify(
                        {"success": False, "error": f"Failed to create table: {str(e)}"}
                    ),
                    500,
                )
