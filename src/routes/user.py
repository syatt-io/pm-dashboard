        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "settings": {
                        ...
                        "fireflies_key_valid": (
                            user.validate_fireflies_api_key()
                            if user.has_fireflies_api_key()
                            else False
                        ),
    ...
    try:
        ...
        try:
            with session_scope() as db_session:
                ...
                db_user.fireflies_api_key_encrypted = (
                    encrypt_api_key_inline(api_key) if api_key.strip() else None
                )

            logger.info(f"Fireflies API key saved for user {user.id}")
            return jsonify(
                {"success": True, "message": "Fireflies API key saved successfully"}
            )
...
def delete_fireflies_api_key(user):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": "Fireflies API key deleted successfully"}
        )
...
def validate_fireflies_api_key_endpoint(user):
    ...
    try:
        ...
        return jsonify({"valid": False, "error": "Failed to validate API key"}), 500
...
def save_google_oauth_token(user):
    ...
    try:
        ...
        if not token_data or not isinstance(token_data, dict):
            return (
                jsonify({"success": False, "error": "OAuth token data is required"}),
                400,
            )
        ...
        if not token_data.get("access_token"):
            return (
                jsonify(
                    {"success": False, "error": "OAuth token must contain access_token"}
                ),
                400,
            )
        ...
        try:
            ...
            return jsonify(
                {"success": True, "message": "Google OAuth token saved successfully"}
            )
...
def delete_google_oauth_token(user):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": "Google OAuth token deleted successfully"}
        )
...
def save_notion_api_key(user):
    ...
    try:
        ...
        try:
            ...
            return jsonify(
                {"success": True, "message": "Notion API key saved successfully"}
            )
...
def delete_notion_api_key(user):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": "Notion API key deleted successfully"}
        )
...
def validate_notion_api_key_endpoint(user):
    ...
    try:
        ...
        is_valid = (
            api_key.startswith("secret_") or api_key.startswith("ntn_")
        ) and len(api_key) > 20
        ...
        return jsonify(
            {
                ...
                "message": (
                    ...
                    if is_valid
                    else "API key format is invalid"
                ),
            }
        )
...
def delete_slack_user_token(user):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": "Slack connection removed successfully"}
        )
    ...
    try:
        ...
        with session_scope() as db_session:
            ...
            extended_prefs = (
                ...
                .filter(UserNotificationPreferences.user_id == db_user.id)
                .first()
            )
...
def update_notification_preferences(user):
    ...
    try:
        ...
        if not data or not isinstance(data, dict):
            return (
                jsonify({"success": False, "error": "Preferences data is required"}),
                400,
            )
        ...
        for field in boolean_fields:
            if field in data and not isinstance(data[field], bool):
                return (
                    jsonify({"success": False, "error": f"{field} must be a boolean"}),
                    400,
                )
        ...
        if "daily_brief_time" in data and not isinstance(data["daily_brief_time"], str):
            return (
                jsonify(
                    {"success": False, "error": "daily_brief_time must be a string"}
                ),
                400,
            )
        ...
        if "timezone" in data and not isinstance(data["timezone"], str):
            return (
                jsonify({"success": False, "error": "timezone must be a string"}),
                400,
            )
        ...
        with session_scope() as db_session:
            ...
            if "notify_daily_todo_digest" in data:
                db_user.notify_daily_todo_digest = bool(
                    data["notify_daily_todo_digest"]
                )

            if "notify_project_hours_forecast" in data:
                db_user.notify_project_hours_forecast = bool(
                    data["notify_project_hours_forecast"]
                )
...
def get_user_escalation_preferences(user):
    ...
    try:
        ...
        with session_scope() as db_session:
            prefs = (
                db_session.query(EscalationPreferences)
...
def update_user_escalation_preferences(user):
    ...
    try:
        ...
        if dm_threshold is not None and dm_threshold < 1:
            return (
                jsonify(
                    {"success": False, "error": "dm_threshold_days must be at least 1"}
                ),
                400,
            )
        ...
        with session_scope() as db_session:
            ...
            if not prefs:
                ...
                prefs = EscalationPreferences(
                    ...
                    enable_channel_escalation=data.get(
                        "enable_channel_escalation", True
                    ),
