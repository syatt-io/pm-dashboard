...
def create_auth_blueprint(db_session_factory, limiter=None):
    ...
    def google_login():
        ...
        try:
            ...
            if not user.can_access():
                logger.info(
                    f"User {user.email} authenticated but pending admin approval (role: {user.role.value if user.role else 'unknown'})"
                )
                return (
                    jsonify(
                        {
    ...
    def refresh_token():
        ...
        try:
            ...
            logger.info(
                f"Token found: {bool(token)} (source: {'header' if auth_header else 'cookie'})"
            )
            ...
            remember_me = (
                request.get_json().get("rememberMe", False)
                if request.get_json()
                else False
            )
            ...
            response = make_response(
                jsonify({"message": "Token refreshed", "token": jwt_token})
            )
    ...
    def update_user_team_settings(current_user, user_id):
        ...
        try:
            ...
            if weekly_hours_minimum is not None:
                try:
                    ...
                    if weekly_hours_minimum < 0:
                        return (
                            jsonify({"error": "weekly_hours_minimum must be >= 0"}),
                            400,
                        )
                except (TypeError, ValueError):
                    return (
                        jsonify({"error": "weekly_hours_minimum must be a number"}),
                        400,
                    )
            ...
            if jira_account_id and jira_account_id != user.jira_account_id:
                existing_user = (
                    db_session.query(User)
                    .filter_by(jira_account_id=jira_account_id)
                    .first()
                )
                if existing_user and existing_user.id != user_id:
                    return (
                        jsonify(
                            {
                                "error": f"Jira Account ID already assigned to {existing_user.name}"
                            }
                        ),
                        400,
                    )
            ...
            if slack_user_id and slack_user_id != user.slack_user_id:
                ...
                if existing_user and existing_user.id != user_id:
                    return (
                        jsonify(
                            {
                                "error": f"Slack User ID already assigned to {existing_user.name}"
                            }
                        ),
                        400,
                    )
            ...
            return jsonify(
                {"message": "User team settings updated", "user": user.to_dict()}
            )
    ...
    def create_user(current_user):
        ...
        try:
            ...
            if not email or not name or not google_id:
                return (
                    jsonify({"error": "email, name, and google_id are required"}),
                    400,
                )
            ...
            if existing_user:
                return (
                    jsonify(
                        {"error": "User with this email or google_id already exists"}
                    ),
                    400,
                )
    ...
    def google_workspace_callback():
        ...
        try:
            ...
            frontend_url = os.getenv("WEB_BASE_URL", "http://localhost:3000")
            return redirect(
                f"{frontend_url}/settings?success=google_workspace_connected"
            )
    ...
    def slack_callback():
        ...
        try:
            ...
            if not token_data.get("ok"):
                logger.error(
                    f"Slack OAuth token exchange failed: {token_data.get('error')}"
                )
                frontend_url = os.getenv("WEB_BASE_URL", "http://localhost:3000")
                return redirect(f"{frontend_url}/settings?error=slack_oauth_failed")
            ...
            try:
                ...
                if not user:
                    logger.error(
                        f"User {user_id} not found during Slack OAuth callback"
                    )
                    raise ValueError("User not found")
                ...
                if slack_user_id:
                    ...
                    logger.info(
                        f"Stored Slack user ID {slack_user_id} for user {user_id}"
                    )
