...
def get_watched_projects(user):
    ...
    try:
        with session_scope() as db_session:
            watched_projects = (
                db_session.query(UserWatchedProject)
        ...
        return jsonify({"success": True, "watched_projects": result})
...
def watch_project(user, project_key):
    ...
    try:
        with session_scope() as db_session:
            ...
            if existing:
                return jsonify(
                    {"success": True, "message": "Already watching this project"}
                )
    ...
    try:
        with session_scope() as db_session:
            ...
            if watched_project:
                ...
                return jsonify(
                    {"success": True, "message": f"Stopped watching {project_key}"}
                )
            else:
                return jsonify(
                    {"success": False, "message": "Project not in watched list"}
                )
            ...
            if user_pref:
                ...
                settings_data = {
                    ...
                    "last_notification_sent": (
                        user_pref.last_notification_sent.isoformat()
                        if user_pref.last_notification_sent
                        else None
                    ),
                }
    ...
    try:
        ...
        with session_scope() as db_session:
            ...
            if user_pref:
                ...
                user_pref.notification_cadence = data.get(
...
def send_project_notification(email):
    ...
    try:
        ...
        sent = asyncio.run(
            notification_service.send_user_notifications(email, force=True)
        )
        ...
        if sent:
            ...
            return jsonify(
                {
                    "success": False,
                    "message": "No changes found or notification not due",
                }
            )
                ...
                mappings.append(
                    {
                        ...
                        "notion_page_ids": (
                            json.loads(notion_page_ids) if notion_page_ids else []
                        ),
                        "github_repos": (
                            json.loads(github_repos) if github_repos else []
                        ),
                        "jira_project_keys": (
...
def update_project_resource_mapping(project_key):
    ...
    try:
        ...
        with engine.begin() as conn:  # Use begin() for automatic transaction management
            ...
            result = conn.execute(
                text(
                    "SELECT project_key FROM project_resource_mappings WHERE project_key = :key"
                ),
                {"key": project_key},
            )
        ...
        return jsonify(
            {"success": True, "message": "Resource mapping updated successfully"}
        )
...
def search_slack_channels():
    ...
    try:
        ...
        response = slack_client.conversations_list(types="public_channel", limit=1000)
...
def search_notion_pages():
    ...
    try:
        ...
        search_result = notion_client.search(
            query=query, filter_type="page", page_size=50
        )
        ...
        for page in pages:
            ...
            has_children = any(
                p.get("parent_id") == page_id for p in pages if p["id"] != page_id
            )
...
def search_jira_projects():
    ...
    try:
        ...
        if not all(
            [settings.jira.url, settings.jira.username, settings.jira.api_token]
        ):
...
def get_project_keywords(project_key):
    ...
    try:
        ...
        with engine.connect() as conn:
            result = conn.execute(
                text(
                    "SELECT keyword FROM project_keywords WHERE project_key = :key ORDER BY keyword"
                ),
...
def get_project_characteristics(user, project_key):
    ...
    try:
        with session_scope() as db:
            ...
            characteristics = (
                db.query(ProjectCharacteristics)
            ...
            if characteristics:
                return jsonify(
                    {"success": True, "characteristics": characteristics.to_dict()}
                )
            else:
        ...
        with session_scope() as db:
            ...
            if characteristics:
                ...
                if "extensive_customizations" in data:
                    characteristics.extensive_customizations = data[
                        "extensive_customizations"
                    ]
