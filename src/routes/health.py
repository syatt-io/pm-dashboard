...
def database_health_check():
    ...
    try:
        ...
        pool_stats["utilization_percent"] = round(
            (total_in_use / total_capacity) * 100, 2
        )
        ...
        status_code = (
            200 if pool_stats["pool_status"] in ["healthy", "warning"] else 503
        )
...
def jira_health_check():
    ...
    try:
        diagnostics = {
            "jira_url": (
                settings.jira.url if hasattr(settings, "jira") else "NOT_LOADED"
            ),
            "jira_username": (
                settings.jira.username if hasattr(settings, "jira") else "NOT_LOADED"
            ),
            "jira_token_set": (
                bool(settings.jira.api_token)
                if hasattr(settings, "jira") and hasattr(settings.jira, "api_token")
                else False
            ),
            "jira_token_length": (
                len(settings.jira.api_token)
                if hasattr(settings, "jira")
                and hasattr(settings.jira, "api_token")
                and settings.jira.api_token
                else 0
            ),
            "jira_token_prefix": (
                settings.jira.api_token[:10] + "..."
                if hasattr(settings, "jira")
                and hasattr(settings.jira, "api_token")
                and settings.jira.api_token
                else None
            ...
            "env_jira_token_prefix": (
                os.getenv("JIRA_API_TOKEN", "")[:10] + "..."
    ...
    try:
        ...
        if not health_status.get("healthy"):
            health_status["warning"] = (
                "Celery workers may be unavailable or not responding"
            )
        ...
        return (
            jsonify(
                {
                    "status": (
                        "healthy" if health_status.get("healthy") else "unhealthy"
                    ),
                    "timestamp": datetime.now().isoformat(),
