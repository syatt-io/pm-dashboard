        ...
        with session_scope() as db:
            ...
            for row in results:
                ...
                if row.slack_channel_ids:
                    all_channels = [
                        ch.strip()
                        for ch in row.slack_channel_ids.split(",")
                        if ch.strip()
                    ]
                ...
                if row.internal_slack_channels:
                    internal_channels = [
                        ch.strip()
                        for ch in row.internal_slack_channels.split(",")
                        if ch.strip()
                    ]
    ...
    try:
        ...
        with session_scope() as db:
            ...
            if not exists:
                return (
                    jsonify(
                        {"success": False, "error": f"Project {project_key} not found"}
                    ),
                    404,
                )
            ...
            db.execute(
                update_query, {"project_key": project_key, "channels": channels_csv}
            )
