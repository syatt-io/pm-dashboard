...
class TestMyProjects:
    ...
    def test_save_user_settings_missing_email(self, mock_session, client):
        ...
        response = client.post(
            "/api/my-projects/user", json={"slack_username": "testuser"}
        )
    ...
    def test_send_test_notification_success(self, mock_async, client, mock_notifier):
        ...
        response = client.post(
            "/api/my-projects/test-notification", json={"email": "test@example.com"}
        )
    ...
    def test_send_project_notification_success(
        self, mock_service_class, mock_async, client
    ):
    ...
    @patch("src.routes.projects.asyncio.run")
    @patch("src.services.project_notifications.ProjectNotificationService")
    def test_send_project_notification_no_changes(
        self, mock_service_class, mock_async, client
    ):
    ...
    @patch("src.routes.projects.asyncio.run")
    @patch("src.services.project_notifications.ProjectNotificationService")
    def test_send_daily_notifications_success(
        self, mock_service_class, mock_async, client
    ):
