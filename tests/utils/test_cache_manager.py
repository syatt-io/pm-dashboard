...
def cache_manager_no_redis():
    ...
    with patch(
        "src.utils.cache_manager.redis.from_url",
...
class TestCacheKeyGeneration:
    ...
    def test_generate_key_with_params(self, cache_manager_with_redis):
        ...
        key = manager._generate_cache_key(
            "meetings", user_id=42, date_range="7d", status="active"
        )
        ...
        assert (
            len(key.split(":")) == 6
        )  # api_cache:meetings:user:42:params:hash (hash is one part)
    ...
    def test_generate_key_params_sorted(self, cache_manager_with_redis):
        ...
        key1 = manager._generate_cache_key(
            "meetings", user_id=None, z="last", a="first"
        )
        key2 = manager._generate_cache_key(
            "meetings", user_id=None, a="first", z="last"
        )
...
class TestCachedEndpointDecorator:
    ...
    def test_decorator_cache_hit(self, flask_app, mock_redis):
        ...
        with patch("src.utils.cache_manager._cache_manager", None):
            with patch(
                "src.utils.cache_manager.redis.from_url", return_value=mock_redis
            ):
                # Mock cache hit
                cached_data = {
                    "data": {"data": "cached"},
