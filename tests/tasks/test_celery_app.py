                ...
                with patch("os.chmod") as mock_chmod:
                    with patch("os.fdopen", mock_open()) as mock_fdopen:
                        with patch("atexit.register") as mock_atexit:
                            ...
                            assert mock_mkstemp.call_args[1] == {
                                "suffix": ".json",
                                "prefix": "gcp-creds-",
                            }
                            ...
                            assert any(
                                "cleanup_gcp_credentials" in str(call)
                            ...
                            with patch("os.path.exists") as mock_exists:
                                with patch("os.unlink") as mock_unlink:
                                    ...
                                    try:
                                        ...
                                        pytest.fail(
                                            f"Cleanup should handle errors silently, but raised: {e}"
                                        )
                ...
                with patch("os.chmod") as mock_chmod:
                    with patch("os.fdopen", mock_open()):
                        with patch("atexit.register"):
                            ...
                            assert (
                                call_args[0][1] == 0o600
                            ), f"Expected 0o600 permissions, got {oct(call_args[0][1])}"
...
class TestCeleryConfiguration:
    ...
    def test_backfill_tasks_inherit_global_time_limit(self):
        ...
        assert (
            backfill_jira.time_limit is None
        ), "backfill_jira should not override time_limit (should be None to inherit global)"
        assert (
            backfill_tempo.time_limit is None
        ), "backfill_tempo should not override time_limit (should be None to inherit global)"
...
class TestBrokerConfiguration:
    ...
    def test_broker_message_retention_configured(self):
        ...
        assert (
            celery_app.conf.broker_transport_options["message_retention_duration"]
            == 604800
        )
