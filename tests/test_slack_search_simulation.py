...
async def test_slack_search():
    ...
    logger.info(
        f"results.summary: {results.summary[:200] if results.summary else 'None'}..."
    )
    logger.info(
        f"results.citations: {len(results.citations) if results.citations else 0} citations"
    )
    logger.info("=" * 80)
    ...
    if results.summary:
        ...
        logger.error(
            "❌ results.summary is None or empty - Slack bot would fall back to streaming"
        )
