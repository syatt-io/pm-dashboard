...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('include_in_forecasting', sa.Boolean(), nullable=False, server_default=sa.text('true')),
    ...
    op.create_index(op.f('ix_project_forecasting_config_include_in_forecasting'), 'project_forecasting_config', ['include_in_forecasting'], unique=False)


def downgrade() -> None:
    """Downgrade schema."""
    op.drop_index(op.f('ix_project_forecasting_config_include_in_forecasting'), table_name='project_forecasting_config')
