...
def upgrade() -> None:
    ...
    if 'projects' in existing_tables:
        ...
        if 'show_budget_tab' not in columns:
            op.add_column('projects', sa.Column('show_budget_tab', sa.Boolean(), nullable=True, server_default='true'))
    ...
    if 'epic_budgets' not in existing_tables:
        op.create_table(
            ...
            sa.UniqueConstraint('project_key', 'epic_key', name='uq_epic_budgets_project_epic')
        )
        op.create_index(op.f('ix_epic_budgets_project_key'), 'epic_budgets', ['project_key'], unique=False)
        op.create_index(op.f('ix_epic_budgets_epic_key'), 'epic_budgets', ['epic_key'], unique=False)
