...
def upgrade() -> None:
    ...
    op.create_index('idx_project_resource_mappings_key', 'project_resource_mappings', ['project_key'], unique=False)

    # Restore project_monthly_forecast table
    op.create_table('project_monthly_forecast',
        sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
        sa.Column('project_key', sa.String(length=50), nullable=False),
        sa.Column('month_year', sa.Date(), nullable=False),
        sa.Column('forecasted_hours', sa.Numeric(precision=10, scale=2), server_default=sa.text('0'), nullable=True),
        sa.Column('actual_monthly_hours', sa.Numeric(precision=10, scale=2), server_default=sa.text('0'), nullable=True),
        sa.Column('created_at', postgresql.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=True),
        sa.Column('updated_at', postgresql.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP'), nullable=True),
    ...
    op.create_index('idx_project_monthly_forecast_project_key', 'project_monthly_forecast', ['project_key'], unique=False)
    op.create_index('idx_project_monthly_forecast_month_year', 'project_monthly_forecast', ['month_year'], unique=False)


def downgrade() -> None:
    """Downgrade schema - drop the restored tables."""
    op.drop_index('idx_project_monthly_forecast_month_year', table_name='project_monthly_forecast')
    op.drop_index('idx_project_monthly_forecast_project_key', table_name='project_monthly_forecast')
    op.drop_table('project_monthly_forecast')

    op.drop_index('idx_project_resource_mappings_key', table_name='project_resource_mappings')
