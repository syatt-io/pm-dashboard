...
def upgrade() -> None:
    ...
    existing_columns = {
        col["name"] for col in inspector.get_columns("processed_meetings")
    }

    # Add new columns only if they don't exist
    if "key_decisions" not in existing_columns:
        op.add_column(
            "processed_meetings", sa.Column("key_decisions", sa.Text(), nullable=True)
        )
    if "blockers" not in existing_columns:
        op.add_column(
            "processed_meetings", sa.Column("blockers", sa.Text(), nullable=True)
        )
    if "analyzed_at" not in existing_columns:
        op.add_column(
            "processed_meetings", sa.Column("analyzed_at", sa.DateTime(), nullable=True)
        )
    ...
    if "tickets_created" not in existing_columns:
        op.add_column(
            "processed_meetings", sa.Column("tickets_created", sa.Text(), nullable=True)
        )
    if "todos_created" not in existing_columns:
        op.add_column(
            "processed_meetings", sa.Column("todos_created", sa.Text(), nullable=True)
        )
