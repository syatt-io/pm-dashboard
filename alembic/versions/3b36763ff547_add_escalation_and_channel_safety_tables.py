...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_proactive_insights_last_escalated_at",
        "proactive_insights",
        ["last_escalated_at"],
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "insight_id",
            sa.String(36),
            sa.ForeignKey("proactive_insights.id"),
            nullable=False,
        ),
        sa.Column(
            "escalation_type", sa.String(50), nullable=False
        ),  # 'dm', 'channel', 'github_comment'
        sa.Column("escalation_level", sa.Integer(), nullable=False),  # 1, 2, 3
        sa.Column(
            "target", sa.String(255), nullable=False
        ),  # User ID, channel ID, or PR URL
        sa.Column(
            "message_sent", sa.Text(), nullable=True
        ),  # Copy of message for audit
    ...
    op.create_index(
        "ix_escalation_history_insight_id", "escalation_history", ["insight_id"]
    )
    op.create_index(
        "ix_escalation_history_created_at", "escalation_history", ["created_at"]
    )
    op.create_index(
        "ix_escalation_history_escalation_type",
        "escalation_history",
        ["escalation_type"],
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "enable_dm_escalation", sa.Boolean(), nullable=False, server_default="true"
        ),
        sa.Column(
            "enable_channel_escalation",
            sa.Boolean(),
            nullable=False,
            server_default="true",
        ),
        sa.Column(
            "enable_github_escalation",
            sa.Boolean(),
            nullable=False,
            server_default="true",
        ),
        sa.Column(
            "dm_threshold_days", sa.Integer(), nullable=False, server_default="3"
        ),
        sa.Column(
            "channel_threshold_days", sa.Integer(), nullable=False, server_default="5"
        ),
        sa.Column(
            "critical_threshold_days", sa.Integer(), nullable=False, server_default="7"
        ),
        sa.Column("created_at", sa.DateTime(), nullable=False),
